...
** Last update Wed Jan 18 10:35:12 2017 
*/
...
char			*free_or_not(char *result, char *buffer, int size)
{
  if (size == -1)
    return (NULL);
  if (end(buffer, 2, 0) != 0)
...
{
  static t_buffer	news = {{0}, 0, 0};
  char			*result;

  result = NULL;
  if (fd >= 0 &&
      (result = put_excedent(news.buffer, &news.idx, &news.size)) != NULL &&
      end(news.buffer, 1, news.idx) != end(news.buffer, 0, news.idx))
    return (result);
  news.idx = 0;
  while (fd >= 0 && READ_SIZE > 0 &&
	 (news.size = read(fd, news.buffer, READ_SIZE)) > 0)
    {
      news.buffer[news.size] = '\0';
      if ((news.idx = end(news.buffer, 0, 0)) != news.size)
	return (my_realloc(result, news.buffer, news.idx));
      else if ((result = my_realloc(result, news.buffer, -1)) == NULL)
	return (NULL);
    }
  return (((news.size != 0 || news.idx != 0) && news.size != -1) ?
	  result : free_or_not(result, news.buffer, news.size));
}
